...
import { 
    ...
    CheckSquare, Folder, AlertCircle, Loader2,
    ChevronRight, MoreVertical
} from "lucide-react";

export default function ProjectmanageModel({ isOpen, onClose, user, fetchProjects, projects: initialProjects }) {
    ...
    const handleRename = async (id) => {
        ...
        const { error } = await supabase
            .from("projects")
            .update({ project_name: editValue.trim() })
            .eq("id", id);
        
        if (!error) {
            setProjects(projects.map(p => p.id === id ? { ...p, project_name: editValue.trim() } : p));
            setEditingId(null);
            ...
            <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                ...
                >
                    ...
                    <div className="p-8 pb-4 flex items-center justify-between">
                        <div>
                            <h2 className="text-2xl font-serif text-paper">Manage Projects</h2>
                            <p className="text-sm text-txt-muted mt-1">{projects.length} total projects</p>
                        </div>
                        ...
                        <div className="space-y-2">
                            {filteredProjects.map((project) => (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        {editingId === project.id ? (
                                            <div className="flex items-center gap-2">
                                                ...
                                                />
                                                <button onClick={() => handleRename(project.id)} className="p-1 text-sage">
                                                    <Check size={16} />
                                                </button>
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-sm font-medium text-paper truncate">{project.project_name}</span>
                                                <span className="text-[10px] text-txt-muted uppercase tracking-wider mt-0.5">
                                                    Modified {new Date(project.created_at).toLocaleDateString()}
                                                </span>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button 
                                            onClick={() => { setEditingId(project.id); setEditValue(project.project_name); }}
                            ...
                            {filteredProjects.length === 0 && (
                                <div className="py-20 flex flex-col items-center text-center">
                                    ...
                                    <p className="text-sm text-txt-muted">No projects found</p>
                                </div>
                            )}
                        </div>
                    </div>
